...
unsigned int nStakeMaxAge = 30 * 24 * 60 * 60; // unlimited
...
const string strMessageMagic = "BREAKBITS Signed Message:\n";
...
int64_t GetProofOfWorkReward(int64_t nFees)
{
            ...
            if(nBestHeight == 1)
            {
            nSubsidy = 1060000 * COIN;
            }
            
            
            else if(nBestHeight > 1 && nBestHeight <= 250)
            {
            nSubsidy = 0 * COIN;
            }

            else if(nBestHeight > 250 && nBestHeight <= 5000)
            {
            nSubsidy = 5 * COIN;
            }
            ...
            else if(nBestHeight > 10000 && nBestHeight <= 15000)
            {
            nSubsidy = 5 / 4 * COIN;
            }

            else if(nBestHeight > 15000 && nBestHeight <= 20000)
            {
            nSubsidy = 5 / 8 * COIN;
            }
...
int64_t GetProofOfStakeReward_V1(int64_t nCoinAge, int64_t nFees)
{
    ...
    if (fDebug && GetBoolArg("-printcreation"))
        printf("GetProofOfStakeReward(): create=%s nCoinAge=%"PRId64"\n", FormatMoney(nSubsidy).c_str(), nCoinAge);
            ...
            else if(nBestHeight > 6000 && nBestHeight <= 50000)
            {
            nSubsidy = nCoinAge * 33 / (365 * 33 + 8) * COIN_YEAR_REWARD * 4;  // 100%
            }
...
bool CheckDiskSpace(uint64_t nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "BREAKBITS", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
...
bool LoadBlockIndex(bool fAllowNew)
{
    ...
    if (mapBlockIndex.empty())
    {
        ...
        const char* pszTimestamp = "HITHERE";
        CTransaction txNew;
        txNew.nTime = 1529391641;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nVersion = 1;
        block.nTime    = 1529391641;
        block.nBits    = bnProofOfWorkLimit.GetCompact();
        block.nNonce   = !fTestNet ? 500066 : 500066;
        ...
        assert(block.hashMerkleRoot == uint256("0xc9b22861c5a781b8b584623f011b64bd59928b6878bcd8f98315fb6aa0eb11ad"));
...
unsigned char pchMessageStart[4] = { 0x2f, 0x45, 0xbc, 0xe0 };
